...
(defn not-prop [x]
  (assoc x :not-prop true))

(defn- input-decl [{:keys [inputs]}]
  (for [{:keys [name type] :as input} inputs
        :when (not (:not-prop input))]
    (case type
      :vec2 (throw
              (Exception.
...
(defn- ensure-quoted [s]
  (if-not (re-matches #"\s*\".*\"\s*" s)
    (str "\"" s "\"")
    s))

(defn tag-string [{:keys [tags transparent]}]
  (let [tags2 (if transparent
                (assoc tags "Queue" "Transparent")
                tags)]
    (apply str
      (for [[k v] tags2]
        (str (ensure-quoted k) " = " (ensure-quoted v) "\n")))))

(defn- transparency-string [{:keys [transparent]}]
  (if transparent
    (line-join
      ["ZWrite Off"
       ""
